...
(function () {
  const STUDIO_I18N = {
    en: {
      ...
      "rename.hint": "Only lowercase letters, digits and hyphens.",
  ...
  function checkEntryPoints(ext, { onSetupDone } = {}) {
    ...
    return new Promise((resolve) => {
      ...
      setupBtn.addEventListener("click", () => {
        close(false);
        openMetaEditModal(ext, () => {
          store.reload().then(async () => {
            const updated = store.extensions.find((e) => e.id === ext.id) || ext;
            const ok = await runPublishFlow(updated, null, { skipEntryCheck: true });
            if (ok && onSetupDone) onSetupDone();
          });
        });
      });
  ...
  function runPublishFlow(ext, prevVersionOrPromise, { skipEntryCheck = false, onPublishDone } = {}) {
    const entryCheck = skipEntryCheck ? Promise.resolve(true) : checkEntryPoints(ext, { onSetupDone: onPublishDone });
    return entryCheck.then((proceed) => {
