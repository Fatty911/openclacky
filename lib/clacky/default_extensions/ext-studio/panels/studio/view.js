...
(function () {
  const STUDIO_I18N = {
    en: {
      ...
      "pub.done.close": "Done",
      "pub.retry": "Try again",
      "pub.readme.step.title": "Add a description (optional)",
      "pub.readme.step.intro": "Help users understand what your extension does. Markdown is supported.",
    ...
    zh: {
      ...
      "pub.done.close": "完成",
      "pub.retry": "重试",
      "pub.readme.step.title": "添加使用说明（可选）",
      "pub.readme.step.intro": "帮助用户了解你的扩展做了什么。支持 Markdown 格式。",
  ...
  // After a successful publish, show the readme + screenshot step inside the
  // same modal. The modal's body is replaced; footer gets Save & Skip buttons.
  // onDone() is called when the user clicks Save or Skip.
  function openReadmeStep(modal, extId, onDone) {
    // Replace modal title
    const titleEl = modal.overlay.querySelector(".studio-modal-title");
    if (titleEl) titleEl.textContent = t("pub.readme.step.title");

    // Build new body
    const intro = el("p", { class: "studio-modal-intro", text: t("pub.readme.step.intro") });
    ...
    // Screenshots section
    const screenshotsWrap = el("div", { style: "margin-top:12px" });
    const screenshotsList = el("div", { style: "display:flex;flex-wrap:wrap;gap:8px;margin-top:8px" });
    const uploadBtn = el("button", { class: "studio-btn studio-btn-ghost", text: t("pub.screenshot.upload"), style: "font-size:12px" });
    const screenshotHint = el("span", { style: "font-size:11px;color:var(--color-text-secondary);margin-left:8px", text: t("pub.screenshot.hint") });
    const screenshotErr = el("p", { class: "studio-modal-status studio-modal-status-error", style: "display:none;margin-top:4px" });
    ...
    fileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        img.src = json.url;
        img.style.cssText = "width:80px;height:60px;object-fit:cover;border-radius:4px;border:1px solid var(--color-border)";
        const removeBtn = el("button", { style: "position:absolute;top:-6px;right:-6px;background:var(--color-danger,#e53e3e);color:#fff;border:none;border-radius:50%;width:16px;height:16px;font-size:10px;line-height:16px;cursor:pointer;padding:0", text: "×" });
        removeBtn.addEventListener("click", () => thumb.remove());
    ...
    screenshotsWrap.appendChild(el("div", { style: "display:flex;align-items:center" }, [uploadBtn, screenshotHint]));
    screenshotsWrap.appendChild(screenshotsList);
    screenshotsWrap.appendChild(screenshotErr);
    screenshotsWrap.appendChild(fileInput);

    const statusEl = el("p", { class: "studio-modal-status", style: "display:none" });

    const newBody = el("div", { class: "studio-modal-body" }, [intro, readmeArea, screenshotsWrap, statusEl]);

    // Replace body content
    const bodyEl = modal.overlay.querySelector(".studio-modal-body");
    if (bodyEl) bodyEl.replaceWith(newBody);
    else modal.overlay.querySelector(".studio-modal").insertBefore(newBody, modal.footer);

    // Replace footer buttons
    modal.footer.innerHTML = "";

    const skipBtn = el("button", { class: "studio-btn studio-btn-ghost", text: t("pub.readme.skip") });
    skipBtn.addEventListener("click", () => { modal.close(); onDone(); });
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      if (!readme) { modal.close(); onDone(); return; }
    ...
    modal.footer.appendChild(skipBtn);
  ...
  function buildReadmeCard(extId, onEdit) {
  ...
  function openReadmeEditModal(extId) {
    ...
    readmeArea.addEventListener("input", () => {
    ...
    async function loadPublished() {
      ...
      try {
        ...
        exts.forEach((e) => {
          ...
          const editReadmeBtn = el("button", { class: "studio-btn studio-btn-ghost studio-btn-sm", text: t("pub.readme.edit") });
          editReadmeBtn.style.marginLeft = "auto";
          editReadmeBtn.addEventListener("click", () => openReadmeEditModal(e.id));
          head.appendChild(editReadmeBtn);
