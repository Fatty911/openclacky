  ...
  function _invokeRender(slot, entry, container, ctx) {
    ...
    try {
      ...
      if (out === undefined) {
        ...
        // A render returning null almost always means the callback used the
        // wrong signature — e.g. `(ctx) => {...}` instead of the required
        // `(container, ctx, runtime)` — so its `ctx.sessionId` guard tripped
        // on the DOM container. Fail loudly instead of showing a blank panel.
        reason =
          "render() returned null. It must be (container, ctx, runtime): mutate " +
          "the container, or return a DOM node / HTML string. Returning null usually " +
          "means the signature is wrong — e.g. (ctx) => ... instead of (container, ctx).";
        console.error(`[Clacky.ext] extension "${entry.extId || "?"}" render for slot ${JSON.stringify(slot)}: ${reason}`);
        crashed = true;
      } else {
        node = out;
      }
