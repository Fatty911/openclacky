...
const ShareView = (() => {
  ...
  function _buildPoster(copy) {
    ...
    ctx.textAlign = "left";
    ctx.font = "800 72px -apple-system, 'PingFang SC', 'Microsoft YaHei', sans-serif";
    ctx.fillText(_brandName(), 64, 140);

    // ── Divider line ──
    ctx.strokeStyle = t.hero;
    ctx.lineWidth = 4;
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(64, 172);
    ...
    ctx.textAlign = "left";
    _drawAutoTextLeft(ctx, text, 64, 210, W - 128, 420);
    ...
    if (url) {
      ...
      ctx.fillStyle = "#ffffff";
      _roundRect(ctx, qrX - 14, qrY - 14, qrSize + 28, qrSize + 28, 16);
      ctx.fill();
      ...
      ctx.fillText(I18n.t("share.poster.scan"), qrX + qrSize / 2, qrY + qrSize + 44);

      // ── Domain — bottom left ──
      ctx.fillStyle = t.scan;
      ctx.textAlign = "left";
      ctx.font = "300 24px -apple-system, sans-serif";
      ctx.fillText(url.replace(/^https?:\/\//, "").replace(/\/$/, ""), 64, H - 56);
    }
  ...
  function _drawAutoTextLeft(ctx, text, x, startY, maxWidth, maxHeight) {
    const sizes = [52, 46, 40, 36, 32, 28, 24];
    let chosen = null;
    for (const size of sizes) {
    ...
    ctx.font = "600 " + chosen.size + "px -apple-system, 'PingFang SC', 'Microsoft YaHei', sans-serif";
    ctx.textAlign = "left";
    let y = startY + chosen.lh * 0.8;
    for (const line of chosen.lines) {
  ...
  function openModal() {
    ...
    const themeChips = THEME_ORDER.map((id) => {
      ...
      const dot = '<span class="share-theme-dot" style="background:' + THEME_DOT[id] + '"></span>';
      return '<button type="button" class="share-theme-chip' + on + '" data-theme="' + id + '"' +
        ...
        dot + '<span class="share-theme-name">' + _esc(I18n.t(th.labelKey)) + '</span></button>';
    }).join("");
      ...
      '<div class="share-modal" role="dialog" aria-modal="true">' +
        ...
        '<div class="share-body">' +
          '<div class="share-poster-wrap">' +
            '<img class="share-poster-img" alt="poster"/>' +
          '</div>' +
    ...
    img.addEventListener("click", () => _downloadPoster((textarea.value || "").trim()));
