...
const Billing = (() => {
  ...
  function _openScorecardShare() {
    ...
    Share.openScorecard({
      period:       _periodLabel(_currentPeriod),
      cacheHitRate: _getCacheHitRate(),
      costStr:      `${_getCurrencySymbol()}${_formatCost(_convertCost(_summary.total_cost))}`,
      tokensStr:    _formatCompact(_summary.total_tokens),
      requests:     _formatNumber(_summary.record_count)
    });
  }
