...
const Sessions = (() => {
  ...
  return {
    ...
    updateInfoBar(s) {
      ...
      // Session ID label — shows a fixed localised "Session file" label
      // rather than the raw UUID. The span itself is the click trigger for
      // the session actions dropdown (download, etc.).
      const sibId = $("sib-id");
      if (sibId) {
        sibId.textContent = I18n.t("sib.id.label");
        sibId.title = s.id || "";
        if (s.id) {
