...
const Sessions = (() => {
  ...
  function _initNewSessionControls() {
    ...
    arrow.addEventListener("click", () => { dd.hidden = false; });
    arrow.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); dd.hidden = false; }
      if (e.key === "Escape") { dd.hidden = true; arrow.focus(); }
    });
  ...
  function _renderSessionItem(container, s) {
    ...
    if (profile && profile !== "general") {
      if (profile === "coding") {
        ...
        const label = (meta && meta.title) ? meta.title : profile;
  ...
  return {
    ...
    openNewSessionModal() {
      ...
      // Populate agent dropdown — built-ins + user overrides + ext-contributed
  ...
  async function _populateAgentDropdown() {
    ...
    try {
      ...
      agents.forEach(a => {
        ...
        let label;
        if (a.source === "default") {
          // Built-ins (general/coding) have i18n keys; fall back to title
          // for any future built-in we haven't translated yet.
          const key = "sessions.modal.agent." + a.id;
          const translated = I18n.t(key);
          label = translated && translated !== key ? translated : (a.title || a.id);
        } else {
          const suffix = a.source === "extension" ? "  (ext)" : "  (custom)";
          label = (a.title || a.id) + suffix;
        }
      ...
      sel.value = match ? previous : (sel.options[0] && sel.options[0].value) || "general";
    } catch (e) {
