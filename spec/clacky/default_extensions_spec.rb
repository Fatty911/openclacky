...
RSpec.describe "ApiExtension#submit_task" do
  ...
  it "submits task to an idle session" do
    ...
    allow(registry).to receive(:update).with("sess-1", pending_task: "do stuff", pending_working_dir: nil)
    ...
    expect(registry).to have_received(:update).with("sess-1", pending_task: "do stuff", pending_working_dir: nil)
  end
